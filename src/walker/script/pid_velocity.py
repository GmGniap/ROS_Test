...
######################################################
    ...
    #####################################################
        ...
        self.ticks_per_meter = rospy.get_param('ticks_meter', 39490.44586)
        ...
        rospy.Subscriber("wheel_vtarget", Float32, self.targetCallback) ##Edit below
        #rospy.Subscriber("lwheel_vtarget", Float32, self.targetCallback)  
        self.pub_motor = rospy.Publisher('motor_cmd',Float32,queue_size=10)  ##Edit below
    ...
    #####################################################
        ...
        while not rospy.is_shutdown() and self.ticks_since_target < self.timeout_ticks:
            ...
            self.calcVelocity()
            self.doPid()
            self.pub_motorL.publish(self.motor)
            self.r.sleep()
    ...
    #####################################################
        ...
        if (self.wheel_latest == self.wheel_prev):
            ...
            rospy.loginfo('>>>>>New Wheel Value ~ %d' % cur_vel)
            self.appendVel(cur_vel)
            self.calcRollingVel()
    ...
    #####################################################
        ...
        #self.motorR = (0.8 * self.error) + (1 * self.integral) + (1 * self.derivative)
